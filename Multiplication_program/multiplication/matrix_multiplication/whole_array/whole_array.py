...
def main():
    ...
    argparser.add_argument("--col_offset", type=int, default=0)
    argparser.add_argument(
        "--dtype_out",
        type=str,
    ...
    with mlir_mod_ctx() as ctx:
        ...
        print(ctx.module)
        print("number of cols being used: ",args.n_aie_cols, file=sys.stderr)
        print("Offset of cols : ",args.col_offset, file=sys.stderr)
...
def my_matmul(
    ...
    if dev == "npu":
        total_cols = col_offset + n_aie_cols
        if total_cols <= 1:
            dev_ty = AIEDevice.npu1_1col
        elif total_cols <= 2:
            dev_ty = AIEDevice.npu1_2col
        elif total_cols <= 4:
            dev_ty = AIEDevice.npu1        # full 4-col device
    else:
    ...
    def device_body():
        ...
        tiles = [
            [tile(col + col_offset, row) for col in range(0, n_aie_cols)] for row in range(0, 6)
        ]
        shim_tiles = tiles[0]
        mem_tiles = tiles[1]
        core_tiles = tiles[2:]
